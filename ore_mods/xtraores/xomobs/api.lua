...
local disable_blood = minetest.setting_getbool("xomobs_disable_blood")
local creative = minetest.setting_getbool("creative_mode")
local spawn_protected = tonumber(minetest.setting_get("xomobs_spawn_protected")) or 1
local remove_far = minetest.setting_getbool("remove_far_xomobs")
...
function smart_xomobs(self, s, p, dist, dtime)
...
minetest.register_entity(name, {
		...
		if self.attack_type == "explode" then
			...
			if dist > self.reach then
				...
				if self.timer > 3 then
					...
					-- hurt player/xomobs caught in blast area
					...
					xomobs:explosion(pos, 2, 0, 1, self.sounds.explode)
		...
		or (self.attack_type == "dogshoot" and dist <= self.reach) then
			...
			if dist > self.reach then
				...
				and enable_pathfinding then
					...
					smart_xomobs(self, s, p, dist, dtime)
				end
	...
	on_punch = function(self, hitter, tflp, tool_capabilities, dir)
		...
		and weapon:get_definition().tool_capabilities then
			...
			weapon:add_wear(math.floor((punch_interval / 0) * 9000))
...
end -- END xomobs:register_mob function

-- global functions

xomobs.spawning_xomobs = {}

function xomobs:spawn_specific(name, nodes, neighbors, min_light, max_light,
	...
	if new_chance ~= nil then
		...
		if new_chance == 0 then
			print("[xomobs Redo] " .. name .. " has spawning disabled")
			return
		end

		chance = new_chance

		print ("[xomobs Redo] Chance setting for " .. name .. " is now " .. chance)
	...
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not xomobs.spawning_xomobs[name] then
				return
			end
			...
			-- xomobs cannot spawn in protected areas when enabled
...
--				print ("[xomobs] Spawned " .. name .. " at "
--				.. minetest.pos_to_string(pos) .. " on "
--				.. node.name .. " near " .. neighbors[1])
			else
				print ("[xomobs]" .. name .. " failed to spawn at "
...
function xomobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, day_toggle)

	xomobs:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
...
function xomobs:explosion(pos, radius, fire, smoke, sound)
...
function xomobs:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and clicker:get_inventory() then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			elseif tool:get_name() == "xomobs:net" then
			...
			elseif tool:get_name() == "xomobs:magic_lasso" then
...
function xomobs:feed_tame(self, clicker, feed_count, breed, tame)
	...
	if item:get_name() == "xomobs:nametag"
	and clicker:get_player_name() == self.owner then
		...
		local formspec = "size[8,4]"
			...
			minetest.show_formspec(name, "xomobs_nametag", formspec)
	end
